    ...
    def test_response_time_measurement(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            response = self.client.post(
                "/anonymize", json={"text": "John Doe test text"}
            )
            end_time = time.time()
    ...
    def test_text_length_impact_on_performance(self):
        ...
        for size in text_sizes:
            ...
            with patch("main.analyzer_engine") as mock_analyzer, patch(
                ...
                response = self.client.post("/anonymize", json={"text": test_text})
    ...
    def test_high_concurrency_stress(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            def make_request(request_id):
                ...
                response = self.client.post(
                    "/anonymize",
                    json={"text": f"user{request_id}@example.com test text"},
                )
    ...
    def test_memory_usage_under_load(self):
        ...
        for i in range(20):
            with patch("main.analyzer_engine") as mock_analyzer, patch(
                ...
                response = self.client.post(
                    "/anonymize",
                    json={"text": f"Test text iteration {i} " * 100},  # Larger text
                )
                assert response.status_code == 200
...
class TestScalabilityLimits:
    ...
    def test_maximum_text_length_performance(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            start_time = time.time()
            response = self.client.post("/anonymize", json={"text": max_text})
            end_time = time.time()
    ...
    def test_many_entities_performance(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            start_time = time.time()
            response = self.client.post(
                "/anonymize", json={"text": "Text with many email addresses " * 50}
            )
            end_time = time.time()
    ...
    def test_rapid_sequential_requests(self):
        ...
        for i in range(num_requests):
            with patch("main.analyzer_engine") as mock_analyzer, patch(
                ...
                start_time = time.time()
                response = self.client.post(
                    "/anonymize", json={"text": f"Test request {i}"}
                )
                end_time = time.time()
        ...
        overall_avg = sum(steady_state) / len(steady_state)

        # Compare averaged halves rather than a single max sample. On a
        # shared CI runner, any one of ~100 individual wall-clock timings can
        # spike from scheduler noise or a GC pause, so asserting on the max
        # of the raw samples flakes regardless of the threshold chosen.
        # Averaging ~49 samples per half smooths that one-off jitter while
        # staying sensitive to a genuine regression (e.g. a resource leak
        # that makes every request progressively slower).
        midpoint = len(steady_state) // 2
        first_half_avg = sum(steady_state[:midpoint]) / midpoint
        second_half_avg = sum(steady_state[midpoint:]) / (
            len(steady_state) - midpoint
        )

        assert second_half_avg < max(first_half_avg * 3, 0.05)
        assert overall_avg < 0.1  # Average should be under 100ms
    ...
    def test_cpu_usage_monitoring(self):
        ...
        for i in range(10):
            with patch("main.analyzer_engine") as mock_analyzer, patch(
                ...
                response = self.client.post(
                    "/anonymize", json={"text": f"CPU intensive test {i}"}
                )
                assert response.status_code == 200
