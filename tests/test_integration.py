    ...
    def test_email_log_anonymization(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            mock_analyzer.analyze.return_value = [
                ...
                RecognizerResult(
                    "EMAIL_ADDRESS", 93, 113, 0.95
                ),  # jane.smith@client.org
                RecognizerResult(
                    "EMAIL_ADDRESS", 175, 197, 0.95
                ),  # john.doe@company.com
                RecognizerResult(
                    "EMAIL_ADDRESS", 275, 305, 0.95
                ),  # invalid.email@nonexistent.domain
            ]

            mock_result = Mock()
            mock_result.text = (
                log_text.replace("john.doe@company.com", "<EMAIL_1>")
    ...
    def test_customer_support_chat_anonymization(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            mock_analyzer.analyze.return_value = [
                ...
                RecognizerResult(
                    "EMAIL_ADDRESS", 230, 252, 0.95
                ),  # sarah.johnson@email.com
                RecognizerResult(
                    "LOCATION", 380, 410, 0.80
            ...
            entity_types = {
                entity["entity_type"] for entity in data["detected_entities"]
            }
    ...
    def test_medical_record_anonymization(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            mock_analyzer.analyze.return_value = [
                ...
                RecognizerResult(
                    "EMAIL_ADDRESS", 130, 152, 0.95
                ),  # j.smith@healthmail.com
            ...
            response = self.client.post(
                "/anonymize",
                json={"text": medical_text, "config": {"strategy": "redact"}},
            )
    ...
    def test_financial_transaction_log_anonymization(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            response = self.client.post(
                ...
                json={
                    "text": financial_text,
                    "config": {"strategy": "hash", "hash_type": "sha256"},
                },
            )
...
class TestMultiLanguageSupport:
    ...
    def test_spanish_text_anonymization(self):
        ...
        spanish_text = (
            "Mi nombre es María García y mi correo es maria.garcia@ejemplo.com"
        )
    ...
    def test_french_text_anonymization(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            mock_result = Mock()
            mock_result.text = (
                "Je m'appelle <PERSONNE> et mon téléphone est <TELEPHONE>"
            )
            mock_anonymizer.anonymize.return_value = mock_result
    ...
    def test_complex_mixed_pii_document(self):
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            response = self.client.post(
                ...
                json={
                    "text": complex_text,
                    "config": {"strategy": "replace", "replacement_text": "[REDACTED]"},
                },
            )
