...
def sample_analyzer_results():
    ...
    return [
        ...
        RecognizerResult(entity_type="EMAIL_ADDRESS", start=20, end=35, score=0.95),
        RecognizerResult(entity_type="PHONE_NUMBER", start=50, end=62, score=0.90),
    ]
...
class TestAnonymizeEndpoint:
    ...
    def test_anonymize_basic_request(
        self, sample_analyzer_results, sample_anonymize_result
    ):
    ...
    def test_anonymize_with_custom_config(
        self, sample_analyzer_results, sample_anonymize_result
    ):
    ...
    def test_anonymize_multiple_languages(
        self, sample_analyzer_results, sample_anonymize_result
    ):
    ...
    def test_anonymize_invalid_language(self):
        ...
        response = client.post(
            "/anonymize", json={"text": "Test text", "language": "unsupported"}
        )
    ...
    def test_anonymize_text_too_long(self):
        ...
        response = client.post("/anonymize", json={"text": long_text})
    ...
    def test_anonymize_engines_not_initialized(self):
        ...
        with patch("main.analyzer_engine", None), patch("main.anonymizer_engine", None):
...
class TestPerformanceAndMetrics:
    ...
    def test_processing_time_measurement(
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            response = client.post(
                "/anonymize", json={"text": "Test text with some PII"}
            )
    ...
    def test_text_length_metrics(
        ...
        with patch("main.analyzer_engine") as mock_analyzer, patch(
            ...
            response = client.post("/anonymize", json={"text": original_text})
